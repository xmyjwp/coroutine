...
private:
    ...
    //当一个定时器被插入到堆的最前面，且m_tickled为false时，m_tickled会被设置为true，
    //并且onTimerInsertedAtFront()会被调用。这样做是为了确保有新的最近到期的定时器时，系统能够及时处理它。
    bool m_tickled = false;
    // 上次检查系统时间是否回退的绝对时间，保存每次执行m_previouseTime()的时间
