...
// 1.首先，代码定义了一组与系统调用函数相同参数和返回类型的函数指针。这些函数指针用于指向原始的系统调用函数。
// 2.接下来，代码声明了一系列与原始系统调用函数同名的函数。这些函数将被重定义以实现hook功能。
// 3.通过使用extern "C"，这些函数声明可以确保在C++环境中进行外部链接时，不会被C++编译器进行名称修饰（name mangling），
//   从而能够正确链接到原始的C系统调用函数。
// 4.Hook技术的主要目的是拦截和修改系统调用的行为。通过将原始的系统调用函数的地址存储在函数指针中，并重新定义同名的系统
//   调用函数，可以在自定义的函数中执行一些额外的操作（如日志记录、性能监控、异步处理等），然后再调用原始的系统调用函数。
extern "C"
{
